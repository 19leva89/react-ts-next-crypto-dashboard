...
import { AddWalletIcon, ChevronUpDownIcon, HamburguerIcon, MoonIcon, SunIcon } from '@/assets/iconComponents'
...
export default function NavBar() {
	...
	return (
		<nav
			className="sticky bg-white dark:bg-dark z-20 top-0 flex justify-between items-center border-b dark:border-gray-700 py-3 px-4 sm:px-6  
        max-[460px]:text-sm"
		>
			<div className="flex items-center gap-14">
				...
				<button
					className="bg-blue-500 dark:bg-blue-600 text-white hidden min-[680px]:flex items-center gap-2 px-5 py-2 rounded-xl
                hover:bg-blue-600"
				>
					<AddWalletIcon />
			...
			<div className="flex gap-3 text-gray-500 dark:text-white">
				<div className="relative">
					<button
						className="flex items-center gap-3 rounded-xl border text-sm
					...
					<ul
						className={`${currencyIsOpen ? 'block' : 'hidden'}
				...
				<button
					...
					onClick={setTheme}
				>
					{theme === themeMode.light ? <MoonIcon className="size-6" /> : <SunIcon className="size-7" />}
				</button>
			</div>
		</nav>
	)
